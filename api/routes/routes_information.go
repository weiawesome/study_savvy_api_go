...
func InitInformationRoutes(r *gin.RouterGroup, sqlRepository *sql.Repository, redisRepository *redis.Repository) {
	...
	r.PUT("", middlewareJwt.JwtSecure(), middlewareJwt.JwtInformation(), requestInformation.MiddleWareInformationEditContent(), (&handlerInformation.HandlerInformationEdit{Service: information.ServiceInformationEdit{Repository: *sqlRepository}}).Handle)
	r.PUT("/password_edit", middlewareJwt.JwtSecure(), middlewareJwt.JwtInformation(), requestInformation.MiddleWarePasswordEditContent(), (&handlerInformation.HandlerPasswordEdit{Service: information.ServicePasswordEdit{Repository: *sqlRepository}}).Handle)
}
